...
- always add both the **topic category with the exam year** (`category-name/YYYY`) and the **exam year tag** (`year/YYYY`) to the `tags:` list in the frontmatter.
  ...
  - **Image format for complex questions:** If the question contains a diagram, flowchart, table, SQL code block, or heavily formatted lists, do NOT type out the question text. Extract a screenshot of the *entire question block* from the PDF into the `Files/` directory and embed it with its vault path, such as `![[Files/{Year}{Season}_FE-{Paper}_Q{QuestionNumber}_full.png]]`.
