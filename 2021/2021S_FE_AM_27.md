...
Category: #information-management/2021  #year/2021
Status: #philnits
...
A studentâ€™s ID, name, and class ID are recorded in the Student table. Which of the
following SQL returns records of all students whose names start with A?

a) SELECT * FROM Student WHERE name LIKE â€™%Aâ€™;
b) SELECT * FROM Student WHERE name LIKE â€˜%A_â€™;
c) SELECT * FROM Student WHERE name LIKE â€˜A_â€™;
d) SELECT * FROM Student WHERE name LIKE â€˜A%â€™;
? 
d) SELECT * FROM Student WHERE name LIKE â€˜A%â€™;
...
- used in aÂ `WHERE`Â clause to search for a specified pattern in a column.
- Has wildcards:
	- percent signÂ `%`Â represents zero, one, or multiple characters
		...
		- positional
	- underscore signÂ `_`Â represents one, single character
