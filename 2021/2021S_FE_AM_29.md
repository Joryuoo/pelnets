...
b) It represents the relationship among nodes with the three (3) elements, i.e., â€œnode,â€ â€œrelationship,â€ and â€œproperty.â€
c) It saves the desired data together with a value that enables this data to be uniquely identified as a pair.
d) The data for one (1) case is called a â€œdocument,â€ and the data structure of each document is unrestricted and can be changed whenever data is added.
? 
c) It saves the desired data together with a value that enables this data to be uniquely identified as a pair.
