...
for p in prompts:
    ...
    prompt = f"""You are an expert IT instructor. Generate the JSON data for PhilNITS PM Question {q} (which includes subquestions {keys_str}) of the 2010S_FE_PM exam.
Your output must be written to `C:\\Users\\Kyle\\Downloads\\pelnets\\.exam_work\\2010S_PM\\review_q{q}.json` using `write_to_file`.

For each subquestion key `k` in {keys_str}:
1. Read the raw text for this subquestion from `C:\\Users\\Kyle\\Downloads\\pelnets\\.exam_work\\2010S_PM\\contexts\\{{k}}.txt` using `view_file`. NOTE: The text for the overall scenario is usually included in the `.1` context.
...
with open("launch_2010s_pm.json", "w", encoding="utf-8") as f:
