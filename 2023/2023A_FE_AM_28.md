...
TheÂ `GROUP BY <attr>`Â clause selects rows having a common attribute value `<attr>` into a smaller set of rows.
- **typically used with aggregate functions**Â 
- for example, `SELECT item_name, SUM(price) FROM items GROUP BY item_name`Â will sum all the prices of each item, then group theÂ `item_name`Â column to their correspondingÂ `SUM(price)`.
