...
Category: #operating-systems/2023  #year/2023
Status: #philnits
...
- While this can be part of a recovery plan, it specifically deals with undoing updates rather than restoring a system to its initial state and performing an **initial program load**. Itâ€™s not about restarting the system from scratch, but rather undoing changes.
...
- This process assumes that the system is already in an operational state and is usually used in the context of database recovery or logs. It doesnâ€™t refer to restoring a system to its original state from scratch, nor does it involve the "initial program load."

### **d) Warm start**

- A **warm start** is when the system restarts or reboots but retains certain parts of its state or data. It might reuse existing data structures or caches from a previous state. While a warm start is faster and doesnâ€™t load everything from scratch, it doesn't fully revert the system to its initial state as a cold start does.
- This means that the system isnâ€™t truly starting from scratch, and some preprocessing might be involved. So this doesnâ€™t fully align with "restoring to initial state" as described in the question.
