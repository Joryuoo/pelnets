...
Which of the following is a method for embedding a malicious java script code in the content sent to a victimâ€™s web browser from a vulnerable website?
...
- **Scenario**:
    ...
    - Imagine you are logged into your online banking account. An attacker sends you an email with a link that appears harmless, but it triggers a hidden request to your bankâ€™s server. Since you're still logged in, the bank processes the attacker's request as if it came from you, allowing unauthorized actions such as transferring money to the attackerâ€™s account.
