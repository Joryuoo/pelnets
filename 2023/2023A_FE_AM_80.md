...
Category: #software/2023  #year/2023
Status: #philnits
...
When a single-license software package is purchased by a company, which of the following is an activity that is usually permitted in conformity with the concept of the WTOâ€™s Agreement on Trade-Related Aspects of Intellectual Property Rights (TRIPS)?

a) Installing the software package on a large number of computers that are used for the 
companyâ€™s business 
b) Making a backup copy of software for limited purposes, such as the licenseeâ€™s personal 
use or a temporary copy for maintenance 
c) Modifying the software package for the companyâ€™s internal use and using it on a large 
number of computers within the company 
d) Placing the software package on the companyâ€™s internal website so that staff members 
...
In particular, TRIPS permits the **making of a backup copy of the software**, provided itâ€™s for **limited purposes** like the licenseeâ€™s personal use, or as a temporary copy used for maintenance or to ensure proper functioning. This exception is meant to ensure that users can continue to operate the software even if the original copy is damaged or becomes inaccessible.
