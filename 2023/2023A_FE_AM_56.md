...
Category: #devops/2023 #year/2023
Status: #philnits
...
Which of the following is the most appropriate characteristic of the â€œbig bangâ€ or all-at-once migration method that is one of the system migration methods?
...
In the **big bang** approach, the entire system is migrated from the old to the new system in a single, large-scale operation at a specific point in time. This means that all data, processes, and users are transitioned simultaneously. There is no parallel operation of the old and new systems after migration â€” the old system is typically shut down entirely.
...
- **b) An additional application is required for connecting the old and new systems**
	...
	- In the big bang method, thereâ€™s no need for such an application because the old system is shut down once the new one goes live.
