...
Category: #cybersecurity/2022  #year/2022
Status: #philnits
...
d) The systemâ€™s operation status is properly checked before commencing full-time operations.
? 
d) The systemâ€™s operation status is properly checked before commencing full-time operations.
## **Explanation:**
...
3. **System Operation Status Check**:  
   After applying a patch, itâ€™s essential to verify that the system is running properly before allowing full-time operations. This helps prevent unexpected failures or performance issues.
...
- **c) Hardware maintenance records are created and managed by system administrators.**  
   Keeping hardware maintenance records is good practice, but itâ€™s unrelated to checking whether the software patch has been successfully applied and the system is operating smoothly.  
...
The appropriate checkpoint during an audit of a control for reducing the risk of system trouble after applying a software patch is to **check the systemâ€™s operation status before commencing full-time operations**. This makes **d) The systemâ€™s operation status is properly checked before commencing full-time operations** the correct answer.
