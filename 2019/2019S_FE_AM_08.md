...
Category: #programming/2019  #year/2019
Status: #philnits
...
1. **Loop 1** iterates over the characters in **X** from **1** to **m**.
    ...
    - Thus, the operation should be: **A: X(k) â†’ Z(k)** (where k varies from 1 to m).
2. **Loop 2** iterates over the characters in **Y** from **1** to **n**.
    ...
    - Thus, the operation should be: **B: Y(k) â†’ Z(m + k)** (where k varies from 1 to n).
