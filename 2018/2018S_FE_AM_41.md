...
Because forensics deals with investigation and not storage, a) can also be ruled out as there is usually no reason for investigators to store a passwordâ€™s hash, especially if itâ€™s not recoverable, and thus unusable in investigation.
...
"To determine whether the duplicate is an exact copy of the original aÂ _hash value_Â is calculated using mathematical computations; here, a cryptographic hash function is used to produce a hash value. If the hash values for the original and copy match, then the contents of the duplicate are the exact same as the original."
