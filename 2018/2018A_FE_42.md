...
Category: #cybersecurity/2018  #year/2018
Status: #philnits
...
a) It allocates a digital signature to an e-mail body by a userâ€™s request. 
b) It enables to synchronize the time by returning an accurate time to a userâ€™s request. 
...
- **Issuing digital certificates**: The CA verifies the identity of the entity requesting a digital certificate (such as a user or server) and then issues a digital certificate. This certificate includes the entityâ€™s public key and serves as proof that the public key belongs to that entity.
...
- Once the CA verifies the identity of the requester, it issues a **digital certificate**. The certificate contains:
    ...
    - The CAâ€™s digital signature, which certifies the authenticity of the certificate.
- The digital certificate binds the requesterâ€™s **public key** to their identity and ensures others can trust that key.
...
- When someone (a client or another server) wants to communicate securely with the certificate holder:
    - They use the **public key** from the digital certificate to **encrypt** data or verify the certificate holderâ€™s **digital signature**.
...
- When a client (e.g., a web browser) receives the digital certificate from a server, it needs to verify the certificateâ€™s authenticity. This involves:
    - **Checking the CAâ€™s signature**: The client verifies the CAâ€™s digital signature on the certificate using the CAâ€™s **public key** (which is usually pre-installed in the clientâ€™s trusted root certificate store).
...
- Once the certificate is validated, the client can trust the serverâ€™s identity (or the userâ€™s identity) and begin secure communication.
