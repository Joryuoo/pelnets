...
a) A program that attempts to identify a userâ€™s password by accessing a server and trying to 
...
c) A program that is installed on a PC against the userâ€™s intention and collects information 
such as the userâ€™s personal information and access history 
d) A program that sequentially accesses TCP ports of a server from an attackerâ€™s PC in 
